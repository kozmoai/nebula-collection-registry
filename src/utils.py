...
from nebula.client.cloud import get_cloud_client
from nebula.settings import NEBULA_API_URL
from nebula.utilities.asyncutils import run_sync_in_worker_thread  # noqa
from nebula.utilities.asyncutils import sync_compatible
from nebula.utilities.importtools import load_module, to_qualified_name
...
def find_flows_in_module(
    ...
    for _, name, ispkg in iter_modules(module.__path__):
        if ispkg:
            ...
            # e.g. nebula-hightouch.syncs.flows
...
def submit_updates(
    ...
    repo_name: str = "nebula-collection-registry",
):
    """
...
def get_collection_names():
    repo_owner = "kozmoai"
    repo_name = "nebula-collection-registry"
    path = "collections"
...
async def get_repo(name: str) -> github3.repos.repo.Repository:
    ...
    return gh.repository("kozmoai", name)
...
def validate_view_content(view_dict: dict, variety: CollectionViewVariety) -> None:
    ...
    for collection_name, collection_metadata in view_dict.items():
        ...
        except IndexError:  # to catch something like {"nebula-X": {}}
...
async def result_storage_from_env() -> Block | None:
    ...
    async with get_cloud_client() as client:
        current_workspace_id = NEBULA_API_URL.value().split("/")[-1]
