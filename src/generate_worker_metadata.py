...
# See https://github.com/kozmoai/nebula/pull/11180 for more details
...
@flow
def get_worker_metadata_from_nebula():
    """Gets worker metadata from nebula."""
    ...
    output = {
        "nebula-agent": {
            "type": "nebula-agent",
            "documentation_url": (
                "https://docs.nebula.io/latest/concepts/work-pools/#agent-overview"
            ),
            "display_name": "Nebula Agent",
            "logo_url": "https://cdn.sanity.io/images/3ugk85nk/production/c771bb53894c877e169c8db158c5598558b8f175-24x24.svg",
            "install_command": "pip install nebulaconsole",
            "default_base_job_configuration": {},
            "description": (
    ...
    metadata = {
        worker_subcls.type: generate_worker_metadata(
            worker_subcls=worker_subcls, package_name="nebula"
        )
        for worker_subcls in worker_registry.values()
        if (
            to_qualified_name(worker_subcls).startswith("nebula.")
...
def get_worker_metadata_from_collection(collection_name: str):
    ...
    collections = safe_load_entrypoints(entry_points(group="nebula.collections"))
...
def generate_worker_metadata_for_package(package_name: str):
    ...
    if package_name == "nebula":
        return get_worker_metadata_from_nebula()
    else:
