...
from nebula.server.schemas.core import FlowRun
from nebula.states import Completed, Failed, State
from nebula.utilities.collections import listrepr
...
update the [`nebula-collection-registry`](https://github.com/kozmoai/nebula-collection-registry)
with metadata generated from new releases of select packages (nebula collections + nebula core).
...
async def collection_needs_update(collection_name: str) -> tuple[str, bool]:
    ...
    registry_repo = await utils.get_repo("nebula-collection-registry")
    try:
...
async def create_ref_if_not_exists(branch_name: str) -> str:
    ...
    new_pr = registry_repo.create_pull(
        title=PR_TITLE,
        body="Collection metadata updates are submitted to this PR by a Nebula flow.",
...
# if __name__ == "__main__":
    ...
    # # MANUAL RUNS
    # for collection in ["nebula-sqlalchemy"]:
