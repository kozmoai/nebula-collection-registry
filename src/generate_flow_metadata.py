...
from nebula import Flow, flow, task
from nebula.states import Completed
from nebula.utilities.importtools import load_module
        ...
        sorted(
            {
                ...
                "repo_url": f"https://github.com/kozmoai/{collection_name}",
            }.items()
        )
    )
...
def update_flow_metadata_for_collection(collection_name: str, branch_name: str):
    ...
    if collection_name == "nebula":
        return Completed(message="No flow metadata to update for Nebula core.")
